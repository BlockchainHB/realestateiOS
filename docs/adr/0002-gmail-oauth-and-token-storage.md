...
One Google mailbox may serve multiple organizations, while Gmail watches and OAuth revocation are mailbox/provider-wide. Connect, watch setup, disconnect, and failed-callback cleanup transitions therefore serialize on the canonical Google account identity. Overlapping setup is rejected rather than queued with an unrepresented issued grant. Disconnect is local for one organization while another active connection remains; watch shutdown and grant revocation occur only when no active organization depends on the mailbox. A pending final revocation blocks reconnect until provider cleanup succeeds. If mailbox identity remains unavailable after a retry, cleanup is deferred rather than risking another organization's provider access.
