...
Watches are selected for renewal two days before expiry and should be scheduled daily. Recovery synchronization calls `history.list` for connections without a recent successful sync. Each cursor stores the time immediately before its Gmail history snapshot was requested. If Google can no longer serve the cursor, synchronization captures a fresh mailbox history anchor, lists only INBOX messages whose sender or subject matches the supported-provider candidate boundary since the prior cursor snapshot, and then replays history from the fresh anchor. This bounded full-sync recovery overlaps the prior synchronization safely, avoids permanent cursor failure and a speculative full-mailbox body scan, and still prevents unrelated message bodies from being retrieved.

One Google mailbox may serve multiple organizations, while Gmail watches and OAuth revocation are mailbox/provider-wide. OAuth start records a short-lived, state-bound connection intent in the private schema. Callback storage and disconnect first serialize on the organization identity; disconnect cancels pending authorization state and intent even when no connection row exists, while callback storage must match the still-current intent before it can start a watch. Connect, initial and maintenance watch setup, disconnect, and failed-callback cleanup also serialize on the canonical Google account identity. Maintenance rechecks that the connection is active after acquiring the provider lock and holds that lock through watch renewal. Every disconnect request rechecks and removes any grant committed by an overlapping reconnect. Disconnect selects and retains the current token bundle only after acquiring the same lock, so concurrent reauthorization cannot leave a newer grant unrevoked. Each newly stored OAuth grant advances an authorization generation, and a refresh may persist only against the generation it originally loaded, so a late refresh cannot overwrite a newer reauthorization. Overlapping initial setup is rejected rather than queued with an unrepresented issued grant. Disconnect is local for one organization while another active connection remains; watch shutdown and grant revocation occur only when no active organization depends on the mailbox. A pending final revocation blocks reconnect until provider cleanup succeeds. If mailbox identity remains unavailable after a retry, cleanup is deferred rather than risking another organization's provider access.
